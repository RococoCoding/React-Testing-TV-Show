...
import { render, rerender, screen, fireEvent, waitFor, getAllByText, getAllByAltText} from '@testing-library/react';
...
test('check episode image and info show up', ()=>{
  const {getAllByText, getAllByAltText} = render(<Episodes episodes={data}/>)
  expect(getAllByText(/Chapter/i)).toHaveLength(2);
  expect(getAllByText(/minutes/i)).toHaveLength(2);
  expect(getAllByText(/season 1, episode/i)).toHaveLength(2);
  expect(getAllByAltText(/Chapter/i)).toHaveLength(2);
});