...
import { render, rerender, screen, fireEvent, waitFor, getAllByText, queryByText} from '@testing-library/react';
import App from './App';
...
test('Check App loads all elements properly and dropdown works', async () => {
  mockFetchShow.mockResolvedValueOnce(data);
  // arrange - render app, load img, title, description, dropdown
  const {getByAltText, getByText, getByTestId, queryByText, findAllByText} = render(<App />);
  getByText(/fetching data/i);
  await waitFor(() => {
    const poster = getByAltText(/stranger things/i);
    const title = getByTestId(/title/i);
    const description = getByText(/a love letter/i);
    expect(queryByText(/episode/i)).toBeNull();
    // const dropdown = getByRole("listbox");
    // expect(dropdown).toBeInTheDocument;
    // userEvent.click(dropdown);
    // const checkSeasons = async ()=> {
    //   await waitFor(() => {
    //     const seasons = getAllByText("Season");
    //   })
    // }
    // checkSeasons();
